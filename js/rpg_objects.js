...
Game_Map.prototype.eraseAllEvents = function() {
    for (id in this._events){
        this._events[id].erase();
    }
};
...
Game_Character.prototype.findDirection8To = function(goalX, goalY) {
    ...
    while (nodeList.length > 0) {
        ...
        for (var j = 0; j < 8; j++) { // 8 directions (4 cardinal + 4 diagonal)
            var direction = [2, 4, 6, 8, 1, 3, 7, 9][j]; // include diagonal directions
            var x2 = $gameMap.roundXWithDirection(x1, direction);
            var y2 = $gameMap.roundYWithDirection(y1, direction);
            var pos2 = y2 * mapWidth + x2;
            ...
            var g2 = g1 + (j < 4 ? 1 : 1.4); // adjust the cost for diagonal moves (sqrt(2) ≈ 1.4)
            var index2 = openList.indexOf(pos2);
